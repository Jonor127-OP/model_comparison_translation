...
class Transformer(nn.Module):
    ...
    def __init__(self, model_dimension, src_vocab_size, trg_vocab_size, number_of_heads, number_of_layers, dropout_probability, log_attention_weights=False, tie_embedding=True):
        super().__init__()

        if tie_embedding:
            self.src_embedding = self.trg_embedding = Embedding(src_vocab_size, model_dimension)
            ...
            self.src_pos_embedding = self.trg_pos_embedding = PositionalEncoding(model_dimension, dropout_probability)
    ...
    def generate_greedy(self, src, src_mask, MAX_LEN):
        ...
        while True:
            ...
            predicted_ids = [index.item() for index in most_probable_last_token_indices]

            for idx, predicted_id in enumerate(predicted_ids):

                if predicted_id == 1:  # once we find EOS token for a particular sentence we flag it
                    is_decoded[idx] = True
